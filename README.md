...
| | |
|--------|--------|
| **Azure IoT Operations:** <img width="140" alt="image" src="https://github.com/Azure/mec-app-solution-accelerator/assets/1712635/3fced6ab-6c54-4120-95f1-11f1ac74f5dc">| Azure IoT Operations (Aio) components used: Azure IoT MQ (aka. E4K) as MQTT broker and Azure IoT AKRI for dynamic provisioning of video cameras stream ingestion. Aio is optional, for a OSS deployment you can use Mosquitto and AKRI-OSS. For production, Aio is preferred. |
| **Kubernetes:** <img width="120" alt="image" src="https://github.com/Azure/mec-app-solution-accelerator/assets/1712635/37b1d0cd-728b-46ea-9349-76dc94bfb153">| Supported: AKS Edge Essentials for Windows and K3s Arc enabled on Ubuntu Linux - Docker-Desktop-Kubernetes only for limited configurations without Azure IoT Operations. |
| **Azure Arc:** <img width="120" alt="image" src="https://github.com/Azure/mec-app-solution-accelerator/assets/1712635/9de20cd4-6247-4604-984b-453cf296922c">| Used with AKS-EE and k3s Arc-enabled for edge-cluster visualization and monitoring from Azure cloud|
...
From a functional or feature perspective, this application has two main modules:
- A Control-Plane app:
  - Cameras provisioning: Allows the application administrator user to dynamicaly provision video IP camera stream ingestion as new cameras are added to the system. 
  - 5G SIM card provisioning (Optional): If using Azure Private 5G Core (AP5GC) the application administrator user can easily and quickly provisiong 5G SIM Cards without having to use the Azure portal.
 
- An Alerts dashboard app:
  - This is the main sub-system of the application. It handles the video ingestion, AI object detection, events publish/subscription and event-handlers that finally show the alerts in a dashboard.
...
<img width="1048" alt="image" src="TBD">
